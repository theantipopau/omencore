...
- **Step 2 (embed the real font, switch the resource): not started.** Roboto Condensed is not a Windows system font — it must ship as an embedded resource (`src/OmenCoreApp/Fonts/`, referenced via a WPF pack URI) with a correct fallback chain, or non-installed machines will silently fall back to a default and look broken. This step requires downloading the actual font files from an external source, which needs explicit user permission (filename/source/size) before it happens — not yet requested. **License: confirmed as SIL Open Font License 1.1**, not Apache 2.0 — a first check against the wrong repository (`googlefonts/roboto-2`, the legacy "classic" Roboto family, genuinely Apache-2.0-licensed but not what Google Fonts actually serves today) gave the wrong answer; the current, canonical distribution (`google/fonts` repo, `ofl/robotocondensed/OFL.txt` — the same file the fonts.google.com download actually ships) is OFL 1.1, copyright 2011 The Roboto Project Authors. Corrected before Step 2 starts. OFL requires font attribution in distributed copies and prohibits selling the font by itself (bundling it in the app is fine) — needs a `LICENSE`/third-party-notices entry once Step 2 lands, and the license text itself should ship alongside the embedded font files, not just be referenced.
