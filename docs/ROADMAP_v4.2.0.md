...
- **OSD tooltip: not yet done.** Deliberately scoped out of this pass to keep it reviewable — the dashboard chip was the highest-value, lowest-risk target since it's already visible during normal use; the OSD is a separate view with its own layout constraints worth a dedicated look.
- **Guided-diagnostics side-by-side comparison: not yet done.** Still open — "WMI BIOS: 81°C / ACPI zone TZ01: 36°C / LHM package: 83°C" shown together in one place would meaningfully shortcut future field reports like this whole investigation. Worth picking up next.
