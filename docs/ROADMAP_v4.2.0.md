...
- **[Exempt — pure correctness]** Replace the "first zone wins, latched forever" logic in `GetAcpiCpuTemperature()` with deliberate zone selection: enumerate *all* zones, score them by name against a real HP zone-name table, and prefer a confirmed CPU zone. Never latch a zone that failed to match anything — re-evaluate each poll until a confident match is found.
- **[Exempt]** When multiple zones are present, record all of them (name + reading) in the diagnostics export. Today the export cannot tell us which zone a misreporting machine actually latched, which is why this bug survived several field reports.
- **[Exempt]** Make the 18°C outlier guard apply even when `_cachedCpuTemp == 0`, by validating against *any* available secondary source (LibreHardwareMonitor package temp, PawnIO MSR) rather than only against the WMI BIOS value. An unvalidatable reading should be surfaced as unvalidated, not silently promoted.
