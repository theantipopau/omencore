...
**GUI (`OmenCore.Avalonia`) parity deliberately not attempted this pass.** `LinuxHardwareService.cs` turned out to be a *third*, fully independent GPU-telemetry implementation (`ReadGpuTemperatureAsync`, same hwmon-only gap) with its own separate GPU-name resolution (`ReadDrmGpuVendorsAsync` + `FormatGpuName`, reading raw PCI vendor/device IDs from `/sys/class/drm/*/device/` — this is exactly where the reporter's `NVIDIA GPU (0x2c59)` came from). Fixing it properly means understanding its own `HardwareStatus`/`GpuUsage` DTO shape and wiring `NvmlInterop` in a second time — a real, separate task, not attempted alongside the CLI fix to avoid rushing either. See "Not Yet Started" below.

**Not a field-validation item.** Read-only telemetry addition — no fan/EC/thermal/OC/UV write path touched, matching the evidence-gate's own scope. It is, however, genuinely unverified in the sense that matters most (an actual run on the reporter's hardware) — this environment has no way to provide that, and it should be treated as "implemented, awaiting confirmation" rather than "done," consistent with how every other Linux fix this cycle without a cooperative tester has been framed.
...
### Fix, then promote, Process and WiFiSSID automation triggers

Companion to the Temperature/Idle promotion above (both now shipped — see "Done"). Two trigger types remain gated, each with a confirmed bug rather than just "not reviewed yet":

- **WiFiSSID** — the fallback SSID-matching path (used when the primary WMI query fails) doesn't check the SSID at all, just whether any wireless interface is up. Needs a real fix — likely the native WiFi API (`wlanapi.dll`) the code's own comment says is missing — before it's safe to expose.
- **Process** — the trigger reads `ProcessMonitoringService.ActiveProcesses`, which only ever contains processes registered via `TrackProcess()`, and that's only ever called from `GameProfileService` for configured Game Profiles. A Process-trigger rule for a non-Game-Profile executable would silently never fire. Needs `AutomationService` to register Process-trigger rule executables with `TrackProcess()` too (or an independent `Process.GetProcessesByName` enumeration) before promoting.

Both are small, well-scoped fixes now that the promotion pattern (validator entry + UI fields + tests) is established from Temperature/Idle — the remaining work is purely fixing each trigger's actual evaluation bug, not building new infrastructure.

### Linux GUI (`OmenCore.Avalonia`) NVML parity

Companion to the #186 GPU-telemetry fix above (CLI side done — see "Done"). `OmenCore.Avalonia/Services/LinuxHardwareService.cs` has its own separate, duplicate GPU-temperature implementation (`ReadGpuTemperatureAsync`, hwmon-only, same NVML gap) and its own separate GPU-name resolution (`ReadDrmGpuVendorsAsync`/`FormatGpuName`, raw PCI vendor/device ID formatting — the source of the `NVIDIA GPU (0x2c59)` display the reporter also flagged). Needs `NvmlInterop` (already built, in `OmenCore.Linux/Hardware/`, which `OmenCore.Avalonia` already references) wired into both places: temperature/power/utilization in place of/ahead of the hwmon read, and NVML's device name preferred over the raw-PCI-ID fallback in `FormatGpuName`. Understand `HardwareStatus`'s existing `GpuUsage`/temperature field shape first rather than assuming it matches the CLI's `SystemStatus` DTO — they're independent types.
