...
- **Process** and **Idle** (`EvaluateProcessTrigger`/`EvaluateIdleTrigger`) — read correctly on inspection (reuse `ProcessMonitoringService.ActiveProcesses`, and `GetLastInputInfo` with the same TickCount64-wraparound-safe pattern used elsewhere), but weren't reviewed as deeply as Temperature/WiFiSSID this pass. **Left gated**, not because a problem was found, but because promoting a trigger type is a one-way UI commitment (real users will build real rules on it) and neither had the same level of scrutiny — a future pass should give them the same review Temperature and WiFiSSID got here before promoting.

**Fix.** Added `TriggerType.Temperature` to `SupportedTriggerTypes`, a validation case in `TryValidate` (threshold required and clamped 1-110°C, condition must be `"Above"`/`"Below"`; `TemperatureSensor` deliberately left optional since `EvaluateTemperatureTrigger` already defaults a null/empty sensor to `"cpu"` — mirroring how every other optional field in this validator already works), and three new bound properties (`TemperatureThreshold`/`TemperatureCondition`/`TemperatureSensor`) plus an `IsTemperatureTrigger` visibility flag on `AutomationRuleEditorItem`, wired through `ToAutomationRule()`/`FromAutomationRule()`. The XAML reuses the exact same 3-column trigger-fields `Grid` the Time/Battery/ACPower triggers already use — Temperature is the first trigger type to actually need all three columns (threshold, condition, sensor), so no layout changes were needed, just three more conditionally-visible `StackPanel`s.

**Tests.** New `AutomationRuleSchemaValidatorTests.cs` (8 tests) — the first test coverage this validator has ever had at all, not just for this change: accepts a fully-specified valid Temperature rule, accepts one with no sensor specified (the optional-field case), rejects threshold values outside 1-110, rejects a missing threshold, rejects an invalid condition string, and a regression guard confirming `TriggerType.Process` (still gated) is rejected with the "not shipped yet" message — so a future change that widens `SupportedTriggerTypes` further gets a deliberate test failure to update, not a silent behavior change. Full suite: 1389/1389 (up from 1381).

**Not a field-validation item.** No new hardware-write path or read path — `EvaluateTemperatureTrigger` and its downstream actions (`FanService.ApplyPreset`, `PerformanceModeService.SetPerformanceMode`) were already shipped and already exercised via `PowerAutomationService`'s own equivalent logic; this only exposes an existing, already-correct evaluation path to a new UI trigger type.
...
### Promote Process/Idle/WiFiSSID automation triggers (or fix and then promote)

Companion to the Temperature promotion above. `WiFiSSID`'s fallback SSID-matching path is a confirmed bug (see "Done" above) and needs a real fix — likely the native WiFi API (`wlanapi.dll`) the code's own comment says is missing — before it's safe to expose. `Process` and `Idle` had no bug found on inspection but weren't reviewed as deeply as Temperature/WiFiSSID; give them the same scrutiny before adding them to `SupportedTriggerTypes`.
