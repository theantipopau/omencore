...
- [~] Accessibility pass: `AutomationProperties` labeling — expanded across five views, still not exhaustive. Scoped to interactive controls only (the actual screen-reader-relevant gap: a WPF control's default UIA name only resolves cleanly from plain-string `Content`, so any icon+text button, unlabeled `ComboBox`, or bare `ToggleButton`/`RadioButton` is effectively silent to a screen reader even though it's visibly labeled).
  ...
  - **Not done:** the per-zone keyboard grid in `LightingView.xaml` (built from `Border` + `MouseBinding`, not real focusable controls — needs an interaction-model change, not an attribute addition) and its temperature/performance-mode color-hex swatches; the remaining ~60 controls in `SettingsView.xaml` that don't fit the simple single-adjacent-toggle row idiom (nested game-profile rule-builder templates, multi-control status rows), plus most `TextBox`/`Slider` labeling beyond what's listed above.
