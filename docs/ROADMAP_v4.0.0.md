...
- [ ] Game-profile automation: event-based process detection (replace ~2s polling) — **not done**, deliberately split off from the window-title item above since it's a separately-risky change (WMI event-subscription lifecycle, harder to unit test deterministically) rather than a mechanical addition.
