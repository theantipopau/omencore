...
>
> **v3.0.1 (2026-03-04):** Ten fix series (A–J) totalling 22+ individual fixes across three test sessions — XAML startup crash, Secure Boot display inversion, Ctrl+Shift+O hotkey regression, capability warning false positive, dispose leaks, AMD GPU field race condition, GUI polish, CpuClock log noise, temperature sensor thread safety, keyboard lighting null service reference, Dashboard INFO log spam, WmiBiosMonitor shutdown exception, MSI Afterburner shared-memory garbage temperature triggering false thermal emergency, thermal protection sanity guard, and COM STA reentrancy in `GetSystemInfo`. This supersedes hotfix1 and hotfix2. See [CHANGELOG_v3.0.1.md](CHANGELOG_v3.0.1.md) for full details.
