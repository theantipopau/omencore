...
**Verified:** build clean (0 warnings) on the first attempt, full suite 948/948 unchanged, live-launched and diff'd the full startup log (timestamps stripped) against this session's own previous live-launch log line-for-line — the entire hardware bring-up section (NVAPI, PawnIO MSR, WMI BIOS monitor, worker-process prelaunch, capability detection, EC access) came back byte-identical except the worker process's PID (expected — different process each run), plus zero exceptions and `Responding=True`. A subsequent full run of that build (left running past this check) shut down cleanly on its own — fan restore, monitor loops, hotkeys, OMEN key interception, automation, OSD, and the watchdog all stopped in order with zero errors in the log — confirming Stage 1 end-to-end, not just at startup.

**Follow-up session — `HardwareBringup` itself wired into DI (still not migrating the 10 downstream fields).** With Stage 1 confirmed sound including a clean shutdown, took the next concrete step named in the Stage 1 plan's preview: added `HardwareBringup? hardwareBringup = null` to `MainViewModel`'s constructor parameters (now 19 optional services), changed the Stage-1 line to `var bringup = hardwareBringup ?? new HardwareBringup(_logging, _config);`, and registered `services.AddSingleton(_ => new HardwareBringup(Logging, Configuration.Config))` in `App.xaml.cs` — same mechanical pattern as every other entry in `ConfigureServices`. This does **not** yet migrate `_fanService`/`_performanceModeService`/`_keyboardLightingService`/`_hardwareMonitoringService`/`_fanCleaningService` or their downstream consumers onto `HardwareBringup` — each of those has its own additional dependencies (config values, `_notificationService`, `_resumeRecoveryDiagnostics`, `_ecOperationCoordinator`) that still need individual analysis; this step only makes `HardwareBringup` itself resolvable through the container.

**Verified:** build clean (0 warnings), full suite 948/948 unchanged, live-launched and confirmed the DI-injected path produces an identical startup log to the fallback path — same hardware bring-up sequence, same timing (all still eager/synchronous within `MainViewModel`'s construction, confirming DI resolution didn't defer anything), zero exceptions, `Responding=True`.
...
### Game-Profile Automation: Window-Title Disambiguation + Restore-Previous-Profile

**Roadmap items:** "event-based process detection (replace ~2s polling) and window-title disambiguation for same-exe-different-game cases" and "restore-previous-profile handling for multiple simultaneously-tracked games" (Phase C).

**Scope note up front:** only the window-title disambiguation and multi-game restore halves were done this pass. Replacing the ~2s poll (`ProcessMonitoringService`) with real WMI `InstanceCreationEvent`/`InstanceDeletionEvent` watchers is a larger, separately-risky change (event-subscription lifecycle, permission/COM-apartment considerations, harder to unit test deterministically) and was deliberately left for its own pass rather than bundled in here.

**Window-title disambiguation:** `GameProfile` gained an optional `WindowTitleContains` field (`Models/GameProfile.cs`) — a case-insensitive substring that, when set, the process's main window title must contain for that profile to match. This solves the case a plain executable-name match can't: a launcher/emulator/runtime (Java-based games under `javaw.exe`, emulator cores, etc.) hosting several distinct "games" under one process name. `GetProcessMatchScore` now returns a 4-tier score (1 = name only, +1 for exact path, +2 for a window-title match) instead of the old 2-tier one, so a profile with a configured title match always outranks a bare name-only profile for the same executable, and a profile with `WindowTitleContains` set that *doesn't* match the running window's title is excluded entirely rather than winning on name alone.

**Restore-previous-profile:** `GameProfileService.OnProcessExited` previously always fired a `GameExit` (restore-defaults) trigger when the active game's process exited, even if a second tracked game was still running alongside it. It now checks `ProcessMonitoringService.ActiveProcesses` for any other currently-running process that matches a profile (reusing the same scoring logic via a new `FindBestMatchingProfile` helper) and switches to that profile's settings instead of restoring defaults — only falling back to `GameExit` when nothing else tracked is still running. `OnProcessDetected` was refactored to share the same single-process matching helper (`FindMatchingProfile`) rather than duplicating the scoring/sorting inline, with no behavior change for the single-game case.

**Verified:** build clean, 5 new tests added (`GameProfileMatchingTests`: window-title-required match/no-match, combined path+title scoring; `GameProfileServiceTests`: window-title disambiguation end-to-end, switch-to-still-running-game, and the no-other-game-running fallback still restores defaults) — full suite 953/953 (948 baseline + 5 new). Live-launched `OmenCore.exe`, confirmed clean startup log with `Game profile system initialized` and no errors.

### Accessibility Pass — LightingView.xaml Finished, SettingsView.xaml Started

**Follow-up to the two earlier Dashboard/Advanced/FanControl/Lighting passes this cycle.** Closes out the remaining `LightingView.xaml` gap flagged in the prior entry and makes a first pass at `SettingsView.xaml`, the largest remaining file (~3,400 lines) that had been explicitly deferred twice already.

**`LightingView.xaml` (~24 more controls, file now fully covered):** the Corsair DPI stage editor's per-stage `Slider` — previously up to 5 visually-identical unlabeled sliders in a row (one per DPI stage) with no way for a screen reader to tell them apart — now announces e.g. "Stage 2 DPI stage" via a binding-driven `AutomationProperties.Name`. Also labeled: the saved-DPI-profile and keyboard-preset `ComboBox`es, the "Apply keyboard colors on startup" toggle, the three previously-silent "Advanced Lighting Effects" toggles (temperature-responsive lighting, performance-mode-synced lighting, throttling indicators — same silent-`ToggleButton` pattern already fixed elsewhere in this file in the prior pass, just in a section that hadn't been reached yet), the Logitech and Razer R/G/B `TextBox` triples (six boxes total — previously three visually-adjacent, identically-typed, completely unlabeled text fields per brand), and the Macro Profiles `ComboBox`. Also disambiguated two same-labeled "Apply to Keyboard" buttons (zone-color apply vs. per-key apply) that would have announced identically to a screen reader despite doing different things. **Still not covered, unchanged from the prior entry:** the per-zone keyboard grid (built from `Border` + `MouseBinding`, not real focusable controls — needs a bigger interaction-model change, not an attribute addition) and the temperature/performance-mode color-hex swatches, left as documented remaining scope rather than rushed.

**`SettingsView.xaml` (50 controls, first pass — not exhaustive):** this file has ~113 `ToggleButton`/`ComboBox` instances and only 3 were labeled going in. Rather than hand-editing each individually, used a scripted pass (Python regex over the XAML) targeting the file's dominant "settings row" idiom — a `Grid.Column="0"` `StackPanel` with a title `TextBlock` immediately followed by a `Grid.Column="1"` `ToggleButton` — which covers the large majority of this file's toggles by construction. The first attempt used a looser pattern and produced several *mismatched* labels (a title from one row silently attached to an unrelated `ToggleButton` several rows down, whenever a row didn't have an immediately-adjacent toggle) — caught by diffing the output before applying, not after; the pattern was tightened to require the `ToggleButton` immediately follow the title `StackPanel`'s closing tag with nothing but whitespace between them, re-run, and every one of the resulting 44 matches was checked against its source XAML context before being applied. Six `ComboBox`es were labeled by hand for the same row idiom (`ComboBox` in place of `ToggleButton`), including disambiguating two pairs that would otherwise have collided ("Fan preset" / "Performance mode" each appearing once under "When on AC Power" and once under "When on Battery").

**Not done this pass:** the remainder of `SettingsView.xaml` — roughly 60 more `ToggleButton`/`ComboBox` controls that don't fit the simple single-adjacent-toggle row idiom (nested rule-builder templates for game-profile automation conditions, driver/BIOS status rows, several multi-control rows), plus most `TextBox`/`Slider` labeling beyond the six RGB-value boxes fixed in `LightingView.xaml`. This file needs at least one more dedicated pass.

**Verified:** build clean (0 warnings across all 6 projects), full suite 953/953, live-launched `OmenCore.exe` and confirmed a clean startup log with no XAML/binding errors. As with every accessibility entry in this cycle, no screen reader was available in this environment to verify the announced names read naturally — flagging that as still-open verification, not claiming it as done.

### Game-Profile Automation: Event-Based Process Detection

**Roadmap item, the half deliberately deferred last pass:** "event-based process detection (replace ~2s polling)."

**What changed:** `ProcessMonitoringService.StartMonitoring()` now tries to subscribe to WMI `__InstanceCreationEvent`/`__InstanceDeletionEvent` on `Win32_Process` (`TryStartWmiEventing`) as the primary detection path. When it succeeds, the existing poll timer doesn't stop — it slows to a 20s reconciliation pass (`ReconciliationPollIntervalMs`) that exists purely to catch anything a dropped/missed WMI event would otherwise leave stale, rather than driving detection itself. If WMI eventing throws for any reason (restrictive WMI ACLs, WMI service unavailable, sandboxed host), `TryStartWmiEventing` catches it, logs a warning, and the service falls back to the exact original polling behavior (2s active / 10s idle) unchanged — this path was deliberately never made a hard dependency.

On a creation event, a newly-launched tracked process's window often doesn't exist yet at the instant WMI reports it — reading `WindowTitle` immediately would starve the window-title disambiguation feature added last pass (`GameProfile.WindowTitleContains`). A one-shot 1.5s settle timer (`WindowTitleSettleDelayMs`) runs before the real `Process.GetProcessById(pid)` lookup that builds the `ProcessInfo` and fires `ProcessDetected` — the old ~2s poll cadence gave this slack for free; event-based detection needed it added back explicitly. Both the event handlers and the reconciliation poll write into the same `ActiveProcesses` dictionary and both check "already present" before firing, so whichever path notices a process first wins with no duplicate `ProcessDetected`/`ProcessExited` firing — this needed no new locking, the existing `ConcurrentDictionary`/`TryAdd`/`TryRemove` usage was already correct for it.

**Threading note:** WMI event callbacks fire on a thread-pool thread, same as `System.Timers.Timer.Elapsed` did for the polling-only path before this change — this is not a new threading concern for downstream consumers (`GameProfileService` et al.), just the same off-UI-thread contract the poll timer already had.

**Not unit-tested (deliberately):** real WMI event timing against a real OS process isn't something that belongs in the deterministic 953-test suite (same reasoning as `PollingSchedulerTests`' fake-clock approach for `DispatcherTimer`-adjacent code — there's no fake WMI seam here to inject, and building one wasn't justified for this scope). Instead this was verified with a throwaway manual test harness this session: tracked `notepad.exe`, launched it for real, and confirmed `IsEventDrivenDetectionActive=true`, `ProcessDetected` firing ~2.6s after launch (well under the 20s reconciliation interval, proving the event path — not the poll fallback — caught it), and `ProcessExited` firing ~620ms after killing the process. That harness surfaced a real Windows 11 quirk worth documenting for whoever touches this next: `Process.Start("notepad.exe")` returns a launcher-stub PID that is *not* the real window's PID (the modern inbox Notepad is reparented) — the WMI event correctly reported the real PID regardless, but a naive test killing `Process.Start`'s returned handle kills the wrong process and looks like a broken deletion path when it isn't. The harness file was deleted after verification, not committed.

**Verified:** build clean, full suite unchanged at 953/953 (no permanent tests added or removed by this change), live-launched `OmenCore.exe` and confirmed the log shows `Event-based process detection active (WMI __InstanceCreationEvent/__InstanceDeletionEvent)` / `Process monitoring started (0 tracked, event-based=True)` with no errors elsewhere in the startup log. Real end-to-end detection latency and correctness confirmed via the manual harness described above.
