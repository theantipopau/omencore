...
**Version:** 4.1.6<br>
**Status:** Code-complete and test-verified in this environment (1097/1097 tests, 0 build warnings); artifacts not yet built or tagged<br>
**Release notes:** [docs/CHANGELOG_v4.1.6.md](docs/CHANGELOG_v4.1.6.md)<br>
**Roadmap:** [docs/ROADMAP_v4.0.0.md](docs/ROADMAP_v4.0.0.md)

v4.1.6 is a patch found while triaging field reports (an exceptionally detailed GitHub #159, a Discord report from board `8DCD`), reviewing a community-contributed branch for board `8D87`, and auditing UI responsiveness. The most significant fixes: `PerformanceModeService` was attempting CPU/GPU EC power-limit writes to register addresses its own code documents as unconfirmed placeholders by default on any board that didn't opt out — a new, dedicated capability flag now defaults this off everywhere. Switching Performance Mode while Max fan mode was active cleared OmenCore's internal tracking without releasing the actual BIOS Max-fan latch, leaving fans stuck at maximum until the app was closed — now fixed. A community member (`tempestnano`) contributed a large, rigorously-evidenced branch reverse-engineering board `8D87`'s real fan range, tachometers, and performance-mode behavior, plus several general correctness fixes (GPU mode detection, fan-verification honesty, a real battery-info provider) that help boards beyond `8D87` — reviewed file-by-file and merged in. Six UI-responsiveness fixes address a standing "laggy UI" complaint: unthrottled full-redraw-on-tick bugs in two chart controls, a broken animation-restart guard, an unnecessary window-wide blur effect, a subscription leak, and a hot-path allocation.
...
- **Adopted:** a community-contributed branch (`tempestnano`) reverse-engineering board `8D87` — real fan range/tachometers, adapter awareness, and several general correctness fixes (firmware-probed V2 fan-command detection instead of a name-substring guess, honest fan-verification/performance-mode logging, a real battery cycle-count provider, a fan-chart units bug). 92 new tests. Full detail on what was and wasn't included in the changelog.
...
| Version | Summary |
|---|---|
| 4.1.6 | Patch release: unconfirmed EC power-limit register writes now blocked by default (safety-relevant, GitHub #159); fans stuck at maximum after switching Performance Mode away from an active Max fan hold now correctly release the BIOS latch (board `8DCD`); GPU Power Boost EC fallback no longer falsely claims "Extended" applied; two GPU Power Boost status-text fixes; a large community-contributed board-`8D87` support/correctness pass (`tempestnano`) covering real fan range/tachometers, adapter awareness, and several general fixes benefiting other boards too; six UI-responsiveness fixes for a standing "laggy UI" complaint. 1097/1097 tests. |
